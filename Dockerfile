...
RUN apt-get update --fix-missing \
    && apt-get install -q -y nagios-nrpe-server nagios-plugins curl bc \
    && apt-get clean \
...
RUN sed -e 's/^allowed_hosts=/#allowed_hosts=/' -i $NAGIOS_CONF_DIR/nrpe.cfg \
    && echo "command[check_load]=$NAGIOS_PLUGINS_DIR/check_load -w 1,1,1 -c 2,2,2" > $NAGIOS_CONF_DIR/nrpe.d/load.cfg \
    && echo "command[check_mem]=$NAGIOS_PLUGINS_DIR/check_mem -f -C -w 12 -c 10 " > $NAGIOS_CONF_DIR/nrpe.d/mem.cfg \
    && echo "command[check_total_procs]=$NAGIOS_PLUGINS_DIR/check_procs -w 500 -c 700 " > $NAGIOS_CONF_DIR/nrpe.d/procs.cfg
